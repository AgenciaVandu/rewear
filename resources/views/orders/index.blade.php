<x-app-layout>
    ...
    <div class="container py-12">
        <section class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-6 text-white">
        ...
        @if ($orders->count())
            <section class="bg-white shadow-lg rounded-lg px-12 py-8 mt-12 text-trueGray-700">
                <h1 class="text-2xl mb-4">Pedidos recientes</h1>
                <ul>
                    @foreach ($orders as $order)
                        <li>
                            <a href="{{ route('orders.show', $order) }}"
                                class="flex items-center py-2 px-4 hover:bg-trueGray-100">
        ...
        @else
            <div class="bg-white shadow-lg rounded-lg px-12 py-8 mt-12 text-trueGray-700 text-center">
                <span class="font-bold text-lg">No cuentas aun con ordenes</span>
            </div>
        @endif
    </div>
